...
Ergänzung zum Spielgefühl (20. September 2026): Der ausblendbare Ziel- bzw. Rückkehrhinweis verwendet den vorhandenen Platz über dem Chat und dieselben Beige-/Violettvariablen. Rückkehrübersichten verwenden normale Spielfenster und mindestens 44 px hohe Aktionen. Die Angriffszusammenfassung gehört in den scrollbaren Bereich der Marschansicht; Fensterkopf und Entsenden bleiben fest. Verlustzahlen für PvP müssen als Beispiel erkennbar bleiben. Nach Belohnungen ist die nächste Aufgabe eine ausdrückliche Auswahl; automatisches Abholen oder Entsenden ist ausgeschlossen. Umsetzung und Abnahme: `ALPHA_GAME_FEEL_2026-09-20.md`.
...
Musik und Klänge stehen als normale helle Karte in den Einstellungen. Separate, beschriftete Musik-/Effektregler und eine Gesamtstummschaltung haben mindestens 44 px hohe Bedienflächen; die Prozentwerte bleiben sichtbar. Hörproben umbrechen auf kleinen Bildschirmen. Farbpalette und Schrift folgen den gemeinsamen Variablen. Audio ist eine Ergänzung: Bestätigungen und Fehler bleiben auch ohne Ton verständlich. Referenzprüfung: `tests/game_audio_app.cjs`.
...
Alle 2D-Truppenporträts verwenden dieselbe Tierpalette für Infanterie, Bogenschützen und Kavallerie: T1 Grau `#8A8D8F`, T2 Braun `#8B5E3C`, T3 Grün `#4F854B`, T4 Blau `#367BC4`, T5 Violett `#9656B7`, T6 Rot `#BF4545`, T7 Orange `#E58132`, T8 Gelb `#F0D34A`, T9 Gold `#C6A04A`, T10 Platinweiß `#F1F3F5`. Rüstungsflächen und Stoffakzente folgen der Stufenfarbe; Haut, Haare, Leder, Holz und Pferdefell behalten natürliche Farben. Die Rahmen verwenden `.troop-tier-frame` mit `data-troop-tier` und die gemeinsamen `--troop-tier-*`-Variablen. Gold bekommt eine doppelte Kante, Platinweiß eine silbergraue Außenkante. Tiernummer, gesperrter Zustand und Auswahl bleiben zusätzlich erkennbar. Die neuen Grafiken liegen unter `assets/art/characters/tier-colors-v1/`; die unabhängigen 3D-Modelle behalten ihre bisherigen Materialien. Prompts: `TROOP_TIER_COLORS.prompts.json`.
...
### Marsch- und Angriffsfenster

Karten-Charms verwenden die freigegebenen Runensteine: Normal in Türkis mit Spiralrune, Episch in Magenta mit Mondsichel, Legendär in Goldorange mit Stern. Der Kristall schwebt langsam über einem festen Steinsockel; der Bodenring pulsiert und drei kleine Lichtpartikel steigen versetzt auf. Formen und Runen unterscheiden die Seltenheiten zusätzlich zur Farbe. Die CSS-Animationen stoppen bei reduzierter Bewegung und pausierter oder ausgeblendeter Darstellung. Assets: `assets/art/map/runes-v1/`, Bildprompts: `CHARM_RUNES.prompts.json`. Die Einsammelvorschau nutzt dieselbe Grafik. Prüfung: `tests/charm_runes_app.cjs` im isolierten Spielstand `--charm-runes`.

Die Kopfzeile der Truppenliste enthält immer die Auswahl gespeicherter Formationen. Ohne gespeicherte Aufstellung bleibt sie sichtbar und zeigt „Keine Formation gespeichert“. Laden einer Formation begrenzt die Mengen auf verfügbare Truppen und Marschkapazität; die Auswahl liegt nicht in der festen Aktionsleiste.

Alle verfügbaren Truppentypen stehen in einer einzigen senkrecht scrollbaren Liste ohne Seitennummern oder Blätterknöpfe. Sortiert wird nach Tier absteigend: zuerst alle T10-Truppen, dann T9 bis T1; innerhalb einer Stufe folgen Infanterie, Fernkämpfer und Kavallerie. Kompakte Zeilen zeigen Porträt mit Tier, Namen, Bestand, Regler, Zahlenfeld und „Max“. Die Zeilen wachsen nicht mit der Fensterhöhe; Scrollen und Serveraktualisierungen erhalten Mengen und Listenposition. Auf großen Bildschirmen stehen Ziel und Beute links, Truppen in der Mitte sowie ausgewählte Armee und Aktionen rechts. Kleine Bildschirme wechseln zwischen „Truppen“ und „Ziel“; die Aktionsleiste bleibt sichtbar. Ausführliche Armeehinweise dürfen unabhängig scrollen. Prüfungen: `tests/march_windows.cjs` und `tests/march_compact_app.cjs` mit allen 30 Truppentypen im isolierten Testspielstand (`--march-roster`).

Die ausgewählte Armee im Marschfenster zeigt ebenfalls einzelne Truppenstufen in derselben Reihenfolge wie die Auswahlliste. Verschiedene Tiers einer Truppenart werden nicht zu einer gemeinsamen Porträtkarte zusammengefasst. Die Karten bleiben kompakt (76 px hoch), statt mit dem Fenster zu wachsen; ab 13 ausgewählten Truppentypen wird das Raster dichter. Zusammenfassung und Hinweise verwenden kleine Abstände, damit die Auswahlübersicht im Desktop- und Tabletformat ohne Scrollen passt. Rahmen und Porträts folgen in Ausbildung, Marsch, Hospital, Dungeon-Auswahl und Kampf-/Spähberichten derselben Stufenpalette.
...
Spähberichte verwenden eine gemeinsame Ansicht für Post und direkte Berichtseinstiege. Nach dem violetten Statusbanner und der Zeitzeile folgen Zielmacht und Mauer, plünderbare Vorräte mit gesichtetem und geschütztem Anteil, Truppenporträts mit Tier und Gesamtzahl, Verstärkungen, Meisterschaft, sechs Reliktplätze und gesichtete Boni. Meisterschaften bewahren ihre Rollenfarben, Relikte ihre Seltenheiten. Angezeigt werden ausschließlich gespeicherte Spähwerte; fehlende ältere Daten und Spähschutz erhalten ausdrückliche Hinweise. Die Liste scrollt senkrecht, während Schließen, „Zur Post“ und Favorit erreichbar bleiben. Handyansichten verwenden zwei Rohstoffspalten und drei Truppenspalten. Prüfung: `tests/scout_report_app.cjs`.
