...
window.ConquerReportShare = function ({api,toast,esc,getState}) {
    ...
    let channel='alliance',playerId=0,summary='',reportId=0,community=null,loading=false,sending=false,token=null,closing=false;
    ...
    function draw(focusHeading=false){
        ...
        dialog.innerHTML=`<header class="cr-detail-heading"><h2 id="report-share-title" tabindex="-1">Bericht teilen</h2><button type="button" class="cr-button" data-share-close aria-label="Teilen schließen">×</button></header><form class="report-share-form"><div class="report-share-scroll"><p>Wohin möchtest du diese Kampfzusammenfassung senden?</p><nav class="cr-share-channels" aria-label="Ziel für den Bericht">${button('alliance','Allianzchat',loading||!hasAlliance)}${button('world','Weltchat',loading)}${button('private','Privat',loading||!players.length)}</nav>${loading?'<p class="cr-share-loading" role="status">Empfänger werden geladen …</p>':`<label class="cr-share-player" ${channel==='private'?'':'hidden'}>Spieler<select name="player_id" required ${channel==='private'?'':'disabled'}><option value="">Spieler auswählen</option>${players.map(player=>`<option value="${Number(player.id)}" ${Number(player.id)===playerId?'selected':''}>${esc(player.username)}</option>`).join('')}</select></label>`}<section class="cr-share-preview" aria-label="Geteilte Zusammenfassung"><strong>Vorschau</strong><p>${esc(summary)}</p></section></div><footer class="report-share-actions"><button type="button" class="cr-button" data-share-close>Abbrechen</button><button type="submit" class="cr-button cr-primary" ${loading||sending?'disabled':''}>${sending?'Wird geteilt …':'Jetzt teilen'}</button></footer></form>`;
    ...
    dialog.addEventListener('submit',async event=>{
        ...
        const payload={action:'chat.send',channel,message:summary,report_id:reportId};if(channel==='private')payload.player_id=player;
        const key=JSON.stringify({world_id:world(),...payload});if(!receipts.has(key))receipts.set(key,crypto.randomUUID());
        sending=true;draw();
        try{const result=await api('community/action',{...payload,request_id:receipts.get(key)});receipts.delete(key);toast(result.message||'Bericht geteilt.');if(dialog.open)dialog.close();}
    ...
    dialog.addEventListener('close',()=>{
        ...
        document.querySelector('.combat-report [data-combat="share"],.combat-report [data-monster="share"]')?.focus({preventScroll:true});
    });
