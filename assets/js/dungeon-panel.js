...
window.ConquerDungeons=function(ctx){
    ...
    function troopInputs(){
        ...
        return `<div class="dungeon-troops">${available.map(([code,count])=>{const def=defs.find(t=>Number(t.code)===Number(code)),name=(def&&ctx.unitName?.(def))||def?.name||'Truppe '+code,image=def?.tier&&Number(def.tier)<=10?`characters/tier-colors-v1/${['infantry','archer','cavalry'][Number(def.type)-1]||'infantry'}-t${Number(def.tier)}-thumb.webp`:'knight.png';return `<label class="dungeon-troop-row"><img class="troop-tier-frame" data-troop-tier="${Number(def?.tier)||0}" src="${base}/assets/art/${image}" alt="" loading="lazy"><span><strong>${esc(name)}</strong><small>${fmt(count)} verfügbar${def?.tier?' · T'+Number(def.tier):''}</small></span><input type="number" inputmode="numeric" name="troop_${Number(code)}" aria-label="Anzahl ${esc(name)}" min="0" max="${Math.min(50000,Number(count))}" step="1" value="0"></label>`;}).join('')||'<p class="dungeon-empty">Keine freien Truppen. Bilde zuerst Einheiten aus.</p>'}</div>`;
    }
    function actions(p){
