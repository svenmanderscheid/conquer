...
try{
    ...
    \Conquer\Db\MigrationSql::apply($pdo,(string)file_get_contents(ROOT_DIR.'/migrations/0107_bug_report_ideas_and_screenshots.sql'));
    ...
    $payload=['operation_key'=>'bug_test_operation_0001','report_type'=>'idea','category'=>'interface','severity'=>'blocking','title'=>'Button bleibt gesperrt','description'=>'Nach der Auswahl bleibt der Hauptknopf weiterhin gesperrt.','reproduction_steps'=>'Fenster öffnen und Eintrag auswählen.','expected_result'=>'Der Knopf wird aktiv.','page_path'=>'/city#inventory','client_context'=>['viewport'=>'390x844','language'=>'de-LU','menu'=>'Inventar'],'screenshot'=>'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNk+A8AAQUBAScY42YAAAAASUVORK5CYII='];
    ...
    $saved=$db->query('SELECT report_type,category,severity,page_path,client_context,screenshot_mime,OCTET_LENGTH(screenshot) screenshot_size FROM bug_reports WHERE id=?',[$first['id']])->fetch();$context=json_decode($saved['client_context'],true);
    checkBug($saved['report_type']==='idea'&&$saved['category']==='interface'&&$saved['severity']==='blocking'&&$saved['page_path']==='/city#inventory'&&$context['viewport']==='390x844'&&$context['menu']==='Inventar'&&$saved['screenshot_mime']==='image/png'&&(int)$saved['screenshot_size']>0,'report stores type, menu, screenshot and safe diagnostics');
