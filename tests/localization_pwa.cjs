...
(async()=>{await new Promise(resolve=>server.listen(0,'127.0.0.1',resolve));const origin='http://127.0.0.1:'+server.address().port;const browser=await chromium.launch({headless:true,channel:process.env.PLAYWRIGHT_CHANNEL||'chrome'});try{
    for(const base of ['', '/conquer']){
        ...
        assert.equal(await page.locator('#template').textContent(),'Niveau 12');
        ...
        await page.evaluate(()=>ConquerLocale.setLocale('en-US'));await page.waitForFunction(()=>document.documentElement.lang==='en');assert.equal(await page.locator('body>main>button[data-action="community-chat-send"]').textContent(),'Send message');assert.equal(await page.locator('[data-locale-select]').getAttribute('aria-label'),'Language');
    ...
    console.log(`PASS ${Object.keys(catalogs.de).length} shared catalog keys, DE/FR/EN persistence, dynamic labels and templates, protected user content, root/subdirectory manifests and PNG dimensions, sanitized bounded static cache, private-route exclusion and French offline fallback. ${out}`);
