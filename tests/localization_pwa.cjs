...
(async()=>{await new Promise(resolve=>server.listen(0,'127.0.0.1',resolve));const origin='http://127.0.0.1:'+server.address().port;const browser=await chromium.launch({headless:true,channel:process.env.PLAYWRIGHT_CHANNEL||'chrome'});try{
    ...
    console.log('PASS shared catalog keys, DE/FR/EN persistence and dynamic labels, protected user content, root/subdirectory manifests and PNG dimensions, sanitized bounded static cache, private-route exclusion and French offline fallback. '+out);
