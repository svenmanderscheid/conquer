...
(async()=>{
 ...
 try{
  ...
  await page.evaluate(defs=>{
   ...
   window.M={offers:[{id:'food_lumber',name:'Holz für den Aufbau',give:{resource:'food',amount:1000},receive:{resource:'lumber',amount:1000}},{id:'gems_lumber',name:'Bauholz des Kristallhändlers',give:{resource:'gems',amount:20},receive:{resource:'lumber',amount:1100}}],history:[],resources:{...S.city,gems:K.profile.gems}};
  ...
  await tab('merchant').click();assert.equal(await page.locator('.shop-merchant-card').count(),2);assert.equal(await page.locator('.shop-merchant-reward img').count(),2);assert.equal(await page.locator('.shop-merchant-card').first().locator('.shop-merchant-reward strong').innerText(),'+1.000 Holz');assert.equal(await page.locator('.shop-merchant-card').first().locator('.shop-merchant-cost strong').innerText(),'1.000');assert(await page.locator('.shop-merchant-action.pays-gems').isVisible());checks.push('Merchant cards show received material art and icon-labelled resource or crystal costs');
