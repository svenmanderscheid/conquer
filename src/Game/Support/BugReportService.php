...
final class BugReportService
{
    ...
    public static function submit(int $playerId,int $worldId,array $input,string $userAgent,string $ip): array
    {
        ...
        try {
            $db->execute('INSERT INTO bug_reports(operation_key,player_id,world_id,category,severity,title,description,reproduction_steps,expected_result,page_path,client_context,user_agent,ip_address) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)',[
                $operation,$playerId,$worldId,$category,$severity,$title,$description,$steps,$expected,$page,
    ...
    private static function context(mixed $value): ?array
    {
        ...
        $allowed=['viewport','screen','language','platform','online','reduced_motion','app_mode','client_time'];$clean=[];
